...
call "%SCRIPT_DIR%..\..\..\Engine\Build\BatchFiles\RunUAT.bat" BuildPlugin -Plugin="%SCRIPT_DIR%NoesisGUI.uplugin" -TargetPlatforms=Android -Package="%TEMP_DIR%" -NoHostPlatform
xcopy "%TEMP_DIR%\Binaries\*" "%SCRIPT_DIR%Binaries" /s /i /y /q
xcopy "%TEMP_DIR%\Intermediate\*" "%SCRIPT_DIR%Intermediate" /s /i /y /q
rmdir /s /q "%TEMP_DIR%"
exit /B 0
