...
#include "NoesisGuiBlueprintFactory.h"
...
UNoesisGuiXamlFactory::UNoesisGuiXamlFactory(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	bEditorImport = true;
	bEditAfterNew = true;
	SupportedClass = UNoesisGuiXaml::StaticClass();
...
public:
	...
	FNoesisGuiEditorResourceProvider(FString InRootFolder, UNoesisGuiXaml* InNoesisGuiXaml)
		: RootFolder(InRootFolder), NoesisGuiXaml(InNoesisGuiXaml)
	{
	...
	UNoesisGuiXaml* ImportXaml(FString XamlPath);
...
UNoesisGuiXaml* FNoesisGuiEditorResourceProvider::ImportXaml(FString XamlPath)
{
	UNoesisGuiXaml* Xaml = NULL;

	FString BasePackageName = FPackageName::GetLongPackagePath(NoesisGuiXaml->GetOutermost()->GetPathName());
	...
	Xaml = LoadObject<UNoesisGuiXaml>(NULL, *XamlObjectPath);
	...
	if (!Xaml)
	{
		...
		Xaml = NewObject<UNoesisGuiXaml>(XamlPackage, FName(*XamlName));
	}
...
UTexture2D* FNoesisGuiEditorResourceProvider::ImportTexture(FString TexturePath)
{
	...
	NoesisGuiXaml->TextureMap.Add(TexturePath, Texture);
	return Texture;
}
...
UFont* FNoesisGuiEditorResourceProvider::ImportFont(FString FontPath)
{
	...
	NoesisGuiXaml->FontMap.Add(FontPath, Font);
	return Font;
}
...
UObject* UNoesisGuiXamlFactory::FactoryCreateBinary(UClass* Class, UObject* Parent, FName Name, EObjectFlags Flags, UObject* Context, const TCHAR* Type, const uint8*& Buffer, const uint8* BufferEnd, FFeedbackContext* Warn)
{
	UNoesisGuiXaml* NoesisGuiXaml = NewObject<UNoesisGuiXaml>(Parent, Class, Name, Flags);
	...
	FNoesisGuiEditorResourceProvider NoesisGuiEditorResourceProvider(*FPaths::GetPath(Filename), NoesisGuiXaml);
	...
	for (auto Element : Elements)
	{
		...
		UNoesisGuiBaseComponent* NoesisGuiComponent = CreateClassFor(Element, NoesisGuiXaml);
		NoesisGuiComponent->ElementName = ElementName ? NsStringToFName(ElementName) : NAME_None;
		NoesisGuiXaml->Components.Add(NoesisGuiComponent);
	}

	return NoesisGuiXaml;
}
